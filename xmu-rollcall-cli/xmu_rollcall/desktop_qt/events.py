...
EVENT_CONTRACTS: dict[str, tuple[int, str]] = {
    ...
    "error": (2, "后台错误文案(message[, worker_token, source_session])"),
    # ---- 登录/恢复/应答 worker 发射 ----
    ...
    "answer_result": (4, "签到应答结果(event_id, ok, detail[, attempted, terminal, task_token, source_session])"),
    "merge_session_cookies": (3, "worker 克隆会话回写(session, worker_account_id[, batch_token])"),
    "notification_result": (3, "通知发送结果(ok, detail[, source_session；None为主动测试])"),

...
def _first_value(data: dict, keys: tuple[str, ...], default=""):
    for key in keys:
        value = data.get(key)
...
def _unwrap_list(payload, keys: tuple[str, ...]) -> list:
...
def _get_json(session, endpoint: str, label: str):
    ...
    # 401 = 会话确定失效：类型化上抛走重新登录引导（retry_request 对其终态短路，
    # 不再对已失效会话重试/继续探测其余端点——与 Android CoursewareClient 行为一致）。
    # 403 保持"资源级无权限"语义（普通 RuntimeError），避免单端点权限问题误触发续登。
...
def _get_modules_cached(session, course_id: str):
    ...
    with _modules_cache_lock:
        _modules_cache[course_id] = (time.monotonic(), payload)
    return payload

...
def _raise_if_session_expired(response):
    """会话过期判定：302 落身份域/登录页（最终 200 HTML）也必须识别为过期。

    requests 对 302 的 PUT 重放会降级为 GET，最终停在 200 登录页——仅凭
    status_code==200 会假成功（界面记「已签到」实际未签成）。send_* 的
...
def circle_intersections(x1, y1, d1, x2, y2, d2):
    ...
    if distance == 0:
        # 两打点坐标解算后重合（同心）：along 计算 2*distance 除零——
        # Android 同构 solveRadarCandidates 的 d==0.0 早退先例
        return None
...
def send_code(in_session, rollcall_id):
    ...
    try:
        ...
        _raise_if_session_expired(response)
...
def send_radar(in_session, rollcall_id):
    ...
    失败路径统一返回 False（不发异常）；会话过期除外——SessionExpiredError
    透传交由上层分流失效路径（过期页落在 200 登录页，仅看状态码会假成功）。
    """
    ...
    try:
        ...
        res_1 = retry_request(
            lambda: in_session.put(url, json=body_1, headers=request_headers, timeout=API_TIMEOUT),
            max_attempts=3,
            delay=2,
            label="radar_1",
        )
        _raise_if_session_expired(res_1)
    except requests.RequestException:
    ...
    try:
        ...
        res_2 = retry_request(
            lambda: in_session.put(url, json=body_2, headers=request_headers, timeout=API_TIMEOUT),
            max_attempts=3,
            delay=2,
            label="radar_2",
        )
        _raise_if_session_expired(res_2)
    except requests.RequestException:
        return False
    ...
    try:
        ...
        res_3 = retry_request(
            lambda: in_session.put(url, json=body_3, headers=request_headers, timeout=API_TIMEOUT),
            max_attempts=3,
            delay=2,
            label="radar_3",
        )
        _raise_if_session_expired(res_3)
    except requests.RequestException:
        return False
    ...
    try:
        ...
        res_4 = retry_request(
            lambda: in_session.put(url, json=body_4, headers=request_headers, timeout=API_TIMEOUT),
            max_attempts=3,
            delay=2,
            label="radar_4",
        )
        _raise_if_session_expired(res_4)
    except requests.RequestException:
        return False

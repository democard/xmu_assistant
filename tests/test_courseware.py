...
class CoursewareTests(unittest.TestCase):
    ...
    def test_detail_thread_session_expired_propagates(self):
        # 体检报告 P1-3：课件详情线程里的会话过期必须向上抛（触发重登引导），
        # 不许被通用 except 转成假「资源缺失」条目。
        class JsonResponse:
            def __init__(self, payload):
                self.payload = payload
        ...
        class ExpiredDetailSession:
            def __init__(self):
                self.detail_requests = 0

            def get(self, url, **kwargs):
                if url.endswith("/courseware-activities"):
                    return JsonResponse([{"id": 11, "type": "material", "title": "Slides"}])
                if url.endswith("/modules"):
                    return JsonResponse({"modules": []})
                if url.endswith("/activities/11"):
        ...
        reset_modules_cache()
        session = ExpiredDetailSession()
        with self.assertRaises(SessionExpiredError):
            fetch_courseware(session, "102746")
        self.assertEqual(session.detail_requests, 1)
...
class ResumeDownloadTests(unittest.TestCase):
    ...
    def test_short_body_against_content_length_keeps_partial(self):
        # 服务端提前断流但连接正常结束：iter_content 不报错，截断文件不得被
        # os.replace 扶正为成品——比对 Content-Length 不符即报错保留 .part
        class ShortResponse:
            status_code = 200
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            with self.assertRaises(RuntimeError) as ctx:
                download_courseware(ShortSession(), self._direct_item(), temp_dir)
            self.assertIn("下载不完整", str(ctx.exception))
            self.assertTrue((Path(temp_dir) / "file.pdf.part").exists())
            self.assertFalse((Path(temp_dir) / "file.pdf").exists())

    def test_content_length_match_promotes_normally(self):
        class FullResponse:
            status_code = 200
            ...
            def iter_content(self, chunk_size):
                yield b"part-onepart-two"  # 恰 16 字节
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            target = download_courseware(FullSession(), self._direct_item(), temp_dir)
            self.assertEqual(target.read_bytes(), b"part-onepart-two")
            self.assertFalse((Path(temp_dir) / "file.pdf.part").exists())
    ...
    def test_fetch_courses_session_expired_short_circuits(self):
        ...
        # 终态短路：第一个端点命中 401 即上抛，不得再对剩余 5 个端点空跑
        self.assertEqual(len(session.urls), 1)

    def test_fetch_courses_non_expired_error_continues_endpoints(self):
        ...
        class OkResponse:
            ...
            def json(self):
                return {"courses": [{"id": "c1", "name": "Course"}]}
        ...
        class MixedSession:
            ...
            def get(self, url, **kwargs):
                self.urls.append(url)
                # 首端点 403（资源级）记账续探，次端点成功返回
                return ForbiddenResponse() if len(self.urls) == 1 else OkResponse()

        courses, _source = fetch_courses(MixedSession())
        self.assertEqual([course.course_id for course in courses], ["c1"])

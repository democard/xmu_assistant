...
class AccountsShapeDefenseTests(unittest.TestCase):
    """accounts 混入非 dict 条目（手工/外部写入）不得穿透 load（2026-08-30 体检 P2-4）。"""
    ...
    def test_load_filters_non_dict_account_entries(self):
        # 字符串/数字/列表条目直接滤除，合法 dict 条目完整保留
        self.config.CONFIG_FILE.write_text(
            '{"accounts":[{"id":1,"name":"","username":"u","password":"",'
            '"rollcall_settings":{}},"corrupt-entry",42,[1,2]],"current_account_id":1}',
            encoding="utf-8",
        )
        loaded = self.config.load_config()
        self.assertEqual(len(loaded["accounts"]), 1)
        self.assertEqual(loaded["accounts"][0]["username"], "u")
        self.assertEqual(self.config.get_next_account_id(loaded), 2)

    def test_load_all_dict_accounts_untouched(self):
        self.config.CONFIG_FILE.write_text(
            '{"accounts":[{"id":1,"name":"","username":"a","password":"",'
            '"rollcall_settings":{}},{"id":2,"name":"","username":"b","password":"",'
            '"rollcall_settings":{}}],"current_account_id":2}',
            encoding="utf-8",
        )
        loaded = self.config.load_config()
        self.assertEqual([a["id"] for a in loaded["accounts"]], [1, 2])

    def test_get_account_by_id_survives_mixed_entries(self):
        # 修复前：非 dict 条目让 get_account_by_id 抛 AttributeError（恢复链路静默失效）
        self.config.CONFIG_FILE.write_text(
            '{"accounts":["junk",{"id":1,"name":"","username":"u","password":"",'
            '"rollcall_settings":{}}],"current_account_id":1}',
            encoding="utf-8",
        )
        loaded = self.config.load_config()
        found = self.config.get_account_by_id(loaded, 1)
        self.assertIsNotNone(found)
        self.assertEqual(found["username"], "u")

    def test_get_next_account_id_tolerates_non_numeric_ids(self):
        # 手工编辑 id 为非数字字符串时，「新增账号」不得 ValueError（非法按 0）
        cfg = {"accounts": [
            {"id": 2, "username": "a"},
            {"id": "corrupted", "username": "b"},
            {"id": 7, "username": "c"},
        ]}
        self.assertEqual(self.config.get_next_account_id(cfg), 8)

    def test_answer_delay_clamped_to_upper_bound(self):
        # 手编巨大延迟 + deadline 缺失事件：延迟应答不得等价于永不提交
        normalized = self.config.normalize_rollcall_settings(
            {"number_delay_max": 10**9, "radar_delay_min": 999999, "radar_delay_max": 999999},
        )
        self.assertEqual(normalized["number_delay_max"], self.config.MAX_ANSWER_DELAY_SECONDS)
        self.assertEqual(normalized["radar_delay_min"], self.config.MAX_ANSWER_DELAY_SECONDS)
        self.assertEqual(normalized["radar_delay_max"], self.config.MAX_ANSWER_DELAY_SECONDS)

    def test_normalize_rejects_wrong_type_settings_sections(self):
        # 手编合法 JSON 但错误类型（数组/字符串）不得穿透 .items()/.update() 崩
        import json as _json

        self.config.CONFIG_FILE.write_text(
            _json.dumps({
                "accounts": [],
                "notification_settings": ["corrupt"],
                "app_settings": "corrupt",
            }),
            encoding="utf-8",
        )
        loaded = self.config.load_config()
        self.assertEqual(
            loaded["notification_settings"]["pushplus"]["enabled"],
            self.config.DEFAULT_NOTIFICATION_SETTINGS["pushplus"]["enabled"],
        )
        self.assertEqual(loaded["app_settings"]["theme_mode"], "system")
        account = {"rollcall_settings": "corrupt"}
        normalized = self.config.normalize_rollcall_settings(account["rollcall_settings"])
        self.assertEqual(
            normalized["poll_interval_seconds"],
            self.config.DEFAULT_ROLLCALL_SETTINGS["poll_interval_seconds"],
        )

    def test_legacy_single_account_migration_keeps_settings(self):
        # 旧单账号格式迁移必须带回 notification_settings/app_settings
        # （此前重建 dict 时两 section 被静默丢弃，重置为默认）
        self.config.CONFIG_FILE.write_text(
            '{"username":"u","password":"p",'
            '"notification_settings":{"system_enabled":false,"pushplus":{"enabled":true,"token":"t"}},'
            '"app_settings":{"theme_mode":"dark"}}',
            encoding="utf-8",
        )
        loaded = self.config.load_config()
        pushplus = loaded["notification_settings"]["pushplus"]
        self.assertTrue(pushplus["enabled"])
        self.assertEqual(pushplus["token"], "t")
        self.assertEqual(loaded["app_settings"]["theme_mode"], "dark")

...
class XmuScheduleModelsTest {
    ...
    @Test
    fun `bom prefixed schedule bodies parse through shared root helper`() {
        // 教务源真实出现过带 BOM 响应（score 侧先例）：xmuJsonRoot 统一剥离
        val bom = "﻿"
        val terms = parseXmuTermCodes(bom + """{"datas":{"kfdxnxqcx":{"rows":[{"XNXQDM":"20261"}]}}}""")
        org.junit.Assert.assertEquals(listOf("20261"), terms)
        org.junit.Assert.assertEquals("08:30", formatXmuTime(830))
    }
    ...
    fun `malformed time values render empty instead of impossible clock`() {
        ...
        org.junit.Assert.assertEquals("00:00", formatXmuTime(0))
    }

...
class MainActivitySourceContractTest {
    ...
    fun `course selection reveals its courseware section without another refresh`() {
        ...
        val selection = page
            ...
            .substringBefore("SectionCard(\"课程课件\")")
    ...
    fun `startup owns monitor launch only after a valid or recovered session`() {
        val source = mainActivitySource()
        val createBlock = source.substringAfter("override fun onCreate").substringBefore("override fun onNewIntent")
        val startupBlock = source.substringAfter("fun startStartupSessionRecovery()").substringBefore("fun recoverExpiredModule(")
    ...
    fun `manual transitions invalidate recovery and set explicit policy`() {
        ...
        val loginBlock = source.substringAfter("onLogin = {").substringBefore("onLogout = {")
        val logoutBlock = source.substringAfter("onLogout = {").substringBefore("onStartMonitor = {")
    ...
    fun `logout clears every page refresh error`() {
        val source = mainActivitySource()
        val logoutBlock = mainScreenSource()
            .substringAfter("onLogout = {", missingDelimiterValue = "")
            .substringBefore("onStartMonitor = {")
    ...
    fun `logout preserves the populated credentials while ending its session`() {
        ...
        assertTrue(
            ...
            "if (busyNow() == \"正在退出登录\") setBusy(\"\")" in logoutBlock,
        )
    ...
    fun `monitor stop invalidates immediately and waits away from the UI callback`() {
        val source = mainActivitySource()
        val stopBlock = mainScreenSource()
            .substringAfter("onStopMonitor = {", missingDelimiterValue = "")
            .substringBefore("onAutoChanged = {")
        ...
        assertTrue(
            ...
            "if (busyNow() == \"正在暂停监控\") setBusy(\"\")" in stopBlock,
        )
    ...
    fun `login completion requires the captured credentials to still match`() {
        ...
        val loginBlock = source
            .substringAfter("onLogin = {", missingDelimiterValue = "")
            .substringBefore("onLogout = {")
    ...
    fun `exam section lives in its own state holder`() {
        ...
        assertTrue("exam re-login must be gated on the epoch", "mayRelogin = { sessionEpoch.isCurrent(session) }" in exam)

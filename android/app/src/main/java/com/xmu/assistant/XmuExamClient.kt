...
internal class XmuExamClient(
    ...
    fun fetchTermExams(termCode: String): XmuTermExamSummary? {
        ...
        JwDebugLog.d("fetch: term=$termCode first=[${first.joinToString { it.exceptionOrNull()?.javaClass?.simpleName ?: "ok" }}]")
    ...
    private fun forceAcademicLoginOnce(): String = try {
        JwDebugLog.d("relogin: start storedLen=${jar.header().length}")
        ...
        jar.clear()
        jar.seed(renewed)
        JwDebugLog.d("relogin: renewedLen=${renewed.length}")
        renewed
    ...
    private fun post(path: String, form: Map<String, String>): String {
        val body = base.formEncode(form)
        val url = "https://jw.xmu.edu.cn/jwapp/sys/studentWdksapApp/modules/wdksap/$path"
        // 与成绩/课表同构：请求前确保应用会话（打开应用入口，跟随重定向）
        ensureAppOpened()
        val startedAt = System.currentTimeMillis()
        val response = base.request(url, "POST", body, referer = EXAM_REFERER, operation = NetworkOperation.EXAM, applyPostHeaders = true)
        JwDebugLog.d("REQ POST $path -> ${response.code} ${System.currentTimeMillis() - startedAt}ms")
    ...
    private companion object {
        ...
        private val EXAM_REFERER = "https://jw.xmu.edu.cn/jwapp/sys/studentWdksapApp/*default/index.do"
    }

    /** exam 与 score 的 request/follow 模板收编于 JwHttpBase（redirect 终止域含 Keycloak）。 */
    private val base = JwHttpBase(
        transport = transport,
        jar = jar,
        networkWrapper = ::ExamNetworkException,
    )
    ...
    private fun ensureAppOpened() {
        ...
        synchronized(this) {
            if (appOpened) return
            try {
                JwDebugLog.d("appOpen: start")
                val opened = base.follow(
                    "https://jw.xmu.edu.cn/jwapp/sys/studentWdksapApp/*default/index.do",
                    operation = NetworkOperation.EXAM,
                    referer = EXAM_REFERER,
                    maxRedirects = MAX_REDIRECTS,
                    // 终止域含 Keycloak（c-identity//auth/realms/xmu/）：原来只有 exam 认、
                    // score 不认的漂移点，现以显式集合单点化（SessionHealth.IDENTITY_MARKERS 对齐）。
                ...
                JwDebugLog.d("appOpen: -> ${opened.code}")
                // 会话建立成功才置位：避免"半初始化"被数据请求消费而间接逼出续登
                appOpened = true
            } catch (error: Throwable) {
                JwDebugLog.d("appOpen: failed ${error.javaClass.simpleName}")
                // 忽略：初始化失败时直接尝试接口请求
            }
        }
    }
    ...
    private fun QueryHttpResponse.isSessionExpired(): Boolean =
        // 401/403+登录页复合判定已单点化（exam/schedule 同构合并；B2 #2）
        JwSessionMarkers.isAuthFailure(code, this)

    /** 解析根对象（三条解析路径共用）：坏 JSON 统一抛 ExamResponseException（B2 收敛）。
     *  BOM 剥离对齐 XmuScoreAutoQueryClient 先例（该源真实出现过带 BOM 响应）。 */
    private fun parseRoot(body: String): JSONObject =
        runCatching { JSONObject(body.removePrefix("\uFEFF")) }.getOrElse { throw ExamResponseException(cause = it) }

    /** datas.&lt;block&gt;.rows 行骨架（原三处逐字重复的取行链，缺段回落空数组）。 */
    private fun rowsOf(root: JSONObject, block: String): JSONArray =
        root.optJSONObject("datas")?.optJSONObject(block)?.optJSONArray("rows") ?: JSONArray()

    private fun parseExams(body: String): List<XmuExam> {
        val root = parseRoot(body)
        val rows = rowsOf(root, "cxxsksap")
        return (0 until rows.length()).mapNotNull { index ->
            val row = rows.optJSONObject(index) ?: return@mapNotNull null
            // optRealString：显式 null 的 KSRWID 会被读成 "null"，多场考试共享该去重键（体检 P1-5）
            val id = row.optRealString("KSRWID").ifBlank { row.optRealString("KSDM") }
    ...
    private fun parseUnarranged(body: String): List<XmuExamUnarranged> {
        val root = parseRoot(body)
        val rows = rowsOf(root, "cxyxkwapkwdkc")
        return (0 until rows.length()).mapNotNull { index ->
            val row = rows.optJSONObject(index) ?: return@mapNotNull null

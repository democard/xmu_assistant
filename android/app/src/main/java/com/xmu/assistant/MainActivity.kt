...
class MainActivity : ComponentActivity() {
    ...
    // 旋转/进程重建后经 onSaveInstanceState 恢复（与组合侧 consumedDeepLinkSeq
    // 配对持久化，见 STATE_DEEP_LINK_SEQ 注释）。
    private var deepLinkSeq by mutableStateOf(1)

    private companion object {
        // deepLinkSeq 的重建恢复键：只持久化组合侧 consumed 而让 seq 重置为 1，
        // 重建后两者错位——守卫「seq <= consumed」会把其后 N-1 次（N=重建前
        // 已消费次数）新深链误判为旧深链回放而吞掉。双计数器同步恢复后，
        // 重建帧 seq==consumed 不消费（防回放性质保持），新 onNewIntent 的
        // seq 必然大于 consumed，照常消费。
        const val STATE_DEEP_LINK_SEQ = "deepLinkSeq"
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        deepLinkSeq = savedInstanceState?.getInt(STATE_DEEP_LINK_SEQ, 1) ?: 1
        ...
        setContent {
            // 加密 prefs 预热门禁（替代原「与组合期首次访问竞速」设计）：
            // MasterKey + EncryptedSharedPreferences 构建较慢，此前 onCreate 的 IO
            // 预热与组合期 remember { AssistantSettings(...) } 竞争同一把初始化锁，
            // 组合输时主线程阻塞等 Keystore（慢 ROM 上拖首帧）。改为 IO 构建完成前
            // 渲染窗口背景占位、主线程永不触碰首次初始化；构建失败异常行为与原先
            // 组合期直构一致（崩溃可见，不静默卡占位）。isReady 快路径保证旋转重建
            // 等单例已缓存的场景不闪占位帧。
            val settingsReady by produceState(initialValue = AssistantSettings.isReady()) {
                if (!value) {
                    value = withContext(Dispatchers.IO) {
                        AssistantSettings(this@MainActivity)
                        true
                    }
                }
            }
            if (settingsReady) {
                XmuAssistantApp(this, openedEventId, openedPage, deepLinkSeq)
            } else {
                Surface {}
            }
        }
    ...
    override fun onSaveInstanceState(outState: Bundle) {
        ...
        outState.putInt(STATE_DEEP_LINK_SEQ, deepLinkSeq)
    }
...
fun XmuAssistantApp(activity: ComponentActivity, openedEventId: String, openedPage: String = "", deepLinkSeq: Int = 1) {
    ...
    // consumedDeepLinkSeq 用 rememberSaveable，Activity 侧 deepLinkSeq 经
    // onSaveInstanceState 同步恢复（双计数器配对，见 STATE_DEEP_LINK_SEQ）：
    // 重建帧 seq==consumed 时跳过，rememberSaveable 恢复的 page 不被旧深链覆盖；
    // 重建后的新 onNewIntent seq 必然大于 consumed，不会被当作回放吞掉。
    ...
    // 课件缓存 JSON 异步解析（8-22 遗留深度版）：org.json 解析课程+课件大 JSON
    ...
    var busy by remember { mutableStateOf(BusyStates.IDLE) }
    ...
    fun show(message: String) {
        toast = message
        toastSeverity = ToastSeverity.INFO
        toastSeq += 1
    }

    // B4 severity 显式化：默认 info（show 不变），失败/告警域逐步迁移到显式级别；
    // 子串推断仍是未迁移调用点的兜底（MainScreen 侧保留）。
    fun showWarning(message: String) {
        toast = message
        toastSeverity = ToastSeverity.WARNING
        toastSeq += 1
    }
    ...
    // 进入签到情况页：最近十次签到 SWR——从未加载过自动拉一次；已加载但超过新鲜
    // 窗口（用户拍板 5 分钟）才后台重校，窗口内静默用缓存秒开不重复拉取。
    // 注意：须放在 rollcall 声明之后（引用其状态）。
    LaunchedEffect(page) {
        if (page != "签到情况") return@LaunchedEffect
        if (!loggedIn || cookieHeader.isBlank()) return@LaunchedEffect
        if (accountTransitionInProgress) return@LaunchedEffect
        val stale = System.currentTimeMillis() - rollcall.historyUpdatedAtMillis >
            ROLLCALL_HISTORY_FRESHNESS_MILLIS
        if (!rollcall.historyLoaded || stale) rollcall.refreshHistory()
    }
    ...
    val schedule = remember {
        ScheduleSectionState(
            ...
            setPendingSessionRetry = { pendingSessionRetry = it },
            clearToast = { toast = "" },
            snapshotInitial = ScheduleSectionState.currentProcessScheduleSnapshot() ?: XmuScheduleSnapshot(),
        )
    }
    ...
    fun clearLoggedOutUi() {
        ...
        // 登录态镜像同步置 false（本路径逐键清理、不走 clearSession）：Widget 周期
    ...
    fun refreshCourses(alreadyRetriedAfterRecovery: Boolean = false): Boolean {
        ...
        workScope.runModuleRequest(
            requestGate = requestGate,
            gateKey = "courses",
            acceptsResult = { sessionEpoch.accepts(session, cookieHeader, loggedIn) },
            ioWork = { CoursewareClient(activity, session.cookieHeader).fetchCourses() },
            onResult = { result ->
                ...
                result.onFailure { error ->
                    if (sessionExpiryAction(error, alreadyRetriedAfterRecovery) == SessionExpiryAction.RECOVER_ONCE) {
                        ...
                        showError("课程读取失败：$coursesRefreshError")
                    }
                }
            },
            releaseLoading = { coursesLoading = false },
        )
        return true
    }
    ...
    fun testNotifications(next: NotificationSettings) {
        ...
        busy = BusyStates.SENDING_TEST_NOTIFICATION
        workScope.launch(Dispatchers.IO) {
            runCatching {
                ...
                .onSuccess { result -> withContext(Dispatchers.Main) {
                    try {
                        if (recovery.accepts(recoveryToken) && sessionEpoch.isOwnerActive(sessionOwner) &&
                            ...
                            if (busy == BusyStates.SESSION_EXPIRED_RELOGIN || busy == BusyStates.CHECKING_LOGIN_STATE) busy = BusyStates.IDLE
                    ...
                    try {
                        if (recovery.accepts(recoveryToken) && sessionEpoch.isOwnerActive(sessionOwner) &&
                            ...
                            if (preserveCachedDataOnFailure) {
                                ...
                                // 恢复失败保留缓存 ≠ 仍在线：登录态镜像同步置 false，
                                // Widget 周期 worker 不得以残留凭据 CAS（幽灵登录守卫）
                                settings.widgetWorkerLoggedIn = false
                                startupSessionReady = false
                            } else {
                            ...
                            pendingModuleRefresh.clear()
                            showWarning("登录已过期，请手动登录")
                            if (busy == BusyStates.SESSION_EXPIRED_RELOGIN) busy = BusyStates.IDLE
                        }
                    } finally {
                        recovery.finishAutoLogin(recoveryToken)
    ...
    fun startStartupSessionRecovery() {
        ...
        if (probeToken == null) {
            startupSessionReady = true
            if (busy == BusyStates.CHECKING_LOGIN_STATE) busy = BusyStates.IDLE
            return
        }
        ...
        if (busy.isBlank()) busy = BusyStates.CHECKING_LOGIN_STATE
        fun complete(action: StartupSessionAction): Boolean {
            ...
            when (action) {
                ...
                StartupSessionAction.AUTO_LOGIN -> {
                    ...
                    invalidateMainSessionUi()
                    if (busy.isBlank() || busy == BusyStates.CHECKING_LOGIN_STATE) busy = BusyStates.SESSION_EXPIRED_RELOGIN
    ...
    fun recoverExpiredModule(retry: ModuleReadRetry) {
        ...
        sessionEpoch.invalidate(sessionOwner)
        invalidateMainSessionUi()
        if (busy.isBlank()) busy = BusyStates.SESSION_EXPIRED_RELOGIN
        val started = startStartupAutoLogin(
            onAcceptedSuccess = {
                when (retry.request) {
                    ModuleReadRequest.ROLLCALL -> rollcall.refresh(alreadyRetriedAfterRecovery = true)
                    ModuleReadRequest.ROLLCALL_HISTORY -> rollcall.refreshHistory(alreadyRetriedAfterRecovery = true)
        ...
        if (!started) {
            settings.markAutoLoginFailed()
            // busy 可能仍是探针置位的 CHECKING 文案：只复位 RELOGIN 会让
            // ToastBar 永久残留 CHECKING 文案（与成功分支 :693 的条件集对齐）
            if (busy == BusyStates.SESSION_EXPIRED_RELOGIN || busy == BusyStates.CHECKING_LOGIN_STATE) busy = BusyStates.IDLE
            show("自动登录未重复执行，请手动登录")
        }
    }
    ...
    // 监控/课程写路径分组（B6 第二批）：HomeActions 与 MainScreen 装配共用一份，
    // 消除两处调用点逐参重复的 lambda 装配；Now 读取器仍独立传入（世代守卫）。
    val monitorWrites = MonitorWritePath(
    ...
    val courseWrites = CoursesWritePath(
    ...
    // 首页登录/登出/停监控协调器：组合作用域普通类，依赖经构造注入（与 SectionState
    // 模式一致）；构造无状态对象，重组重建开销可忽略且始终闭包最新读写器。
    ...
    MainScreen(
        ...
        identity = IdentityUiState(username, password, loggedIn),
        setUsername = { username = it },
        setPassword = { password = it },
        notificationSettings = notificationSettings,
        onNotificationSettingsSaved = { notificationSettings = it },
        status = StatusUiState(toast, toastSeverity, busy, downloadingCount),
        accountTransitionInProgress = accountTransitionInProgress,
        monitor = MonitorUiState(
            monitorRunning,
            monitorConsecutiveFailures,
            monitorLastCheckMillis,
            monitorLastError,
            monitorTransitionInProgress,
            monitorTransitionId,
        ),
        monitorWrites = monitorWrites,
        ...
        courseState = CoursesUiState(courses, selectedCourse, coursesLoading, coursesRefreshError),
        courseWrites = courseWrites,

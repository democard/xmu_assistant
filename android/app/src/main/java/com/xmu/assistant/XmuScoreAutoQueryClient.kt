...
class XmuScoreAutoQueryClient internal constructor(
    ...
    private fun loginAndGetToken() {
        ...
        try {
            ...
            JwDebugLog.d("login: page code=${page.code} saltLen=${salt.length} ltLen=${extractInput(page.body, "lt").length} len=${page.body.length}")
            JwDebugLog.d("login: pageHead=${page.body.take(500).replace('\n', ' ').replace('\r', ' ')}")
            ...
            JwDebugLog.d("login: posted final=${posted.url.substringAfter("xmu.edu.cn")} code=${posted.code}")
            ...
            JwDebugLog.d("login: appShow final=${appShow.url.substringAfter("xmu.edu.cn")} code=${appShow.code} jarLen=${jar.header().length}")
    ...
    /** request/follow 模板收编于 JwHttpBase；终止域保持现状清单（未含 Keycloak，
     *  成绩链路语义冻结）；异常类型与响应装饰逐字对齐原实现。 */
    private val base = JwHttpBase(transport = transport, jar = jar)
    ...
    private fun follow(
        ...
        base.followDecorated(
            url,
            method,
            body,
            operation = operation,
            referer = SCORE_REFERER,
            maxRedirects = MAX_REDIRECTS,
            // 重定向循环耗尽：若终点仍停在统一登录域（ids/authserver），说明是会话过期反复 bounce，
            // 应触发续登（抛会话失效）；若循环发生在教务域内部，属服务端坏跳转（数据性问题），
            // 归为格式异常，避免白打一次 CAS 登录（风控红线）。
            loginTerminators = JwSessionMarkers.SCORE_FOLLOW_TERMINATORS,
            onLoginTerminus = { ScoreSessionExpiredException() },
            onExhaustedNonLogin = { ScoreJsonFormatException("教务跳转次数过多") },
            responseDecorator = { current, response -> scoreResponseOf(current, response) },
        )

    /** 四字段构造单点（request/responseDecorator 原各写一份逐字相同构造，B2 收敛）。 */
    private fun scoreResponseOf(url: String, response: QueryHttpResponse): ScoreResponse =
        ScoreResponse(url, response.code, response.location, response.body)
    ...
    private fun request(
        ...
        // 模板本体（含 UA/Referer/Sec-Fetch 等八件）在 JwHttpBase.request；score 无
        // POST 变体头、无 IOException 包装（与原实现逐字一致）。
        val response = base.request(url, method, body, referer = SCORE_REFERER, operation = operation)
        return scoreResponseOf(url, response)
    }
    ...
    companion object {
        ...
        private const val SCORE_REFERER = "https://jw.xmu.edu.cn/new/index.html"

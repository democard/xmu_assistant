...
/** 课表取数注入点（ScheduleSectionState 的测试 seam）：默认实现见下方
 *  fetchScheduleWithNetworkRetry，生产装配方不传，行为逐字不变。 */
internal fun interface ScheduleFetcher {
    suspend fun fetch(
        username: String,
        password: String,
...
internal class ScheduleSectionState(
    ...
    /** 网络取数注入点（测试 seam，照 CoursewareSectionState.createCoursewareClient 范式）：
     *  默认值即正式实现 fetchScheduleWithNetworkRetry，生产装配方不传该参数，
     *  行为与加参数前逐字一致；测试借它注入会话过期/成功两种取数结果。
     *  用 fun interface 而非函数类型：Kotlin 函数类型调用不支持具名实参，
     *  而 mayRelogin = { sessionEpoch.isCurrent(session) } 这行是契约测试锚定的
     *  源码文本（自动续登必须挂在当前世代上），必须原样保留。 */
    private val fetchSchedule: ScheduleFetcher = ScheduleFetcher { u, p, c, m ->
    ...
    fun refresh(alreadyRetriedAfterRecovery: Boolean = false): Boolean {
        ...
        scope.runModuleRequest(
            ...
            gateKey = "schedule",
            // 账号复核折叠进统一谓词（B1）：换号必推进 epoch 使 accepts 为假，语义等价
            acceptsResult = {
                sessionEpoch.accepts(session, cookieHeader(), loggedIn()) &&
            ...
            onResult = { result ->
                result.onSuccess { refreshResult ->
                    ...
                    // 避免回退到磁盘旧缓存
                    updateProcessScheduleSnapshot(snapshot)
                    syncScheduleWidget(
                        activity,
                        newEntries,
                        newTermCode,
                ...
                result.onFailure { error ->
                    if (sessionExpiryAction(error, alreadyRetriedAfterRecovery) == SessionExpiryAction.RECOVER_ONCE) {
                        ...
                        setPendingSessionRetry(ModuleReadRetry(ModuleReadRequest.SCHEDULE))
                    } else {
                        refreshError = refreshFailureMessage(error)
                        if (entries.isEmpty()) {
                            showError("课表读取失败：$refreshError")
                        } else {
            ...
            // 恢复路径（alreadyRetriedAfterRecovery）统一复位 transition（无条件段，
            // 登出竞态下也执行），先于守卫释放 loading——与原内层 finally 执行序一致
            onFinally = {
                if (alreadyRetriedAfterRecovery) {
                    setAccountTransitionInProgress(false)
                    if (busy() == BusyStates.SESSION_EXPIRED_RELOGIN) setBusy(BusyStates.IDLE)
                }
            },
    ...
    fun applySnapshot(initial: XmuScheduleSnapshot) {
        ...
        updateProcessScheduleSnapshot(initial)
    }
    ...
    fun clearAll() {
        ...
        loading = false
        refreshError = ""
        updateProcessScheduleSnapshot(null)
    }
    ...
    companion object {
        ...
        @Volatile
        private var processScheduleSnapshot: XmuScheduleSnapshot? = null

        fun currentProcessScheduleSnapshot(): XmuScheduleSnapshot? = processScheduleSnapshot

        fun updateProcessScheduleSnapshot(snapshot: XmuScheduleSnapshot?) {
...
internal suspend fun fetchScheduleWithNetworkRetry(
    ...
    repeat(3) { attempt ->
        try {
            ...
            // 先落局部 val 再使用：两处 !! 收敛为一处，语义不变
            val establishedCookie = jwCookie!!
            val result = XmuScheduleClient(establishedCookie)
                .fetchCurrentSchedule(username)
            return ScheduleRefreshResult(
                jwCookie = establishedCookie,

...
- [双端实现与验收交接](../scripts/attendance-threshold-integration.md)。下方 v1.6.9 下载不包含这些开发版变更。
...
- **签到情况 / 监控**：前台服务后台轮询，检测到签到后展示课程、类型和状态；支持数字签到和雷达签到自动处理；页面含「正在进行」与「历史签到（最近十次，本人明细核实）」两个卡片，顶部刷新一键刷两块；开发版接入人数、比例、数字码与自定义门槛
...
- [项目根 README](../README.md)：当前发布版本、v1.6.9 修复、构建方式与验收范围。
